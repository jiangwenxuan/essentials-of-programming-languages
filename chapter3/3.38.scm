...
(define-datatype expression expression?
  ...
  (call-exp (rator expression?)
            (rand expression?))
  (cond-exp (exp1 (list-of expression?))
...
(define trans-l
  (lambda (exp1 senv)
    (if (null? exp1)
        '()
        (cons (translation-of (car exp1) senv)
              (trans-l (cdr exp1) senv)))))
...
(define cond-cal
  (lambda (exp1 exp2 nameless-env)
    (if (null? exp1)
        ...
        (let ((val1 (expval->bool (value-of (car exp1) nameless-env))))
          (if val1
              (value-of (car exp2) nameless-env)
              (cond-cal (cdr exp1) (cdr exp2) nameless-env))))))
...
(define value-of
  (lambda (exp nameless-env)
    (cases expression exp
      ...
      (cond-exp (exp1 exp2)
                (cond-cal exp1 exp2 nameless-env))
...
(define l3 "let x = 4 in cond zero?(x) ==> -(x, 1) zero?(-(x, 4)) ==> -(x, 3) end")
