...
; my answer is wrong at first, so I see Yukang's code, and change a little
; this is the code
...
(define extend-env-list
  (lambda (list-env env)
    (if (null? list-env)
        env
        (extend-env-list (cdr list-env) (extend-env (caar list-env)
                                                    (cdar list-env)
                                                    env)))))
...
(define let-cal
  (lambda (ids exps env)
    (let-cal-collect ids exps '() env)))

(define let-cal-collect
  (lambda (list-id list-exp list-env env)
    (if (null? list-id)
        list-env
        (let-cal-collect (cdr list-id)
                         (cdr list-exp)
                         (cons (cons (car list-id)
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (ids exps body)
               (value-of body (extend-env-list (let-cal ids exps env) env)))
      (cons-exp (exp1 exp2)
