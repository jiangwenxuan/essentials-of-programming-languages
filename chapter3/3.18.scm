...
(define-datatype expression expression?
  ...
  [unpack-exp (ids (list-of identifier?))
              (exp1 expression?)
              (exp2 expression?)])
...
(define unpack-cal
  (lambda (ids exp1 env)
    (cond
      ((and (null? ids) (expval->bool (expval-null? exp1)))
       env)
      ((and (not (null? ids)) (not (expval->bool (expval-null? exp1))))
       (unpack-cal (cdr ids) (expval-cdr exp1) (extend-env (car ids) (expval-car exp1) env)))
      (else
       (eopl:error 'unpack "num of ids and num of list of exps are not eq")))))
...
;                    (newline)
;                    (display ids)
                    (value-of body (unpack-cal ids val1 env)))))))
...
(define l3 "let u = 7 in unpack x y = cons(u, cons(3, emptylist)) in -(x, y)")
;(define l4 "unpack x = cons(6, cons(3, emptylist)) in -(x, y)")
(display (run l3))
;(newline)
;(display (run l4))